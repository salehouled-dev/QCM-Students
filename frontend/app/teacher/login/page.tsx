...
export default function TeacherLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/auth/teacher`, { password });
      if (res.data.success) {
        // Enregistrer le token dans localStorage pour sécuriser les autres pages
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-20">
      ...
      <div className="card text-center py-10 px-8 shadow-xl">
        ...
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Accès Enseignant</h1>
        <p className="text-slate-600 mb-8">Veuillez entrer votre mot de passe pour accéder à votre espace de gestion.</p>
